...
def has_collided(old_state, updated_field):
    return not all([old_state[i] == updated_field[i] for i in old_state.keys()])
...
def num_field(board):
    num_grid = [[0 for _ in range(COLUMNS)] for _ in range(ROWS)]

    for ind_x, r in enumerate(board):
        for ind_y, c in enumerate(r):
            if c != (255, 255, 255):
                num_grid[ind_x][ind_y] = 1
...
def make_move(move, piece, collision):
    if action_space[move] == 'down':
        ...
        while not collided:
            done = make_move(pygame.K_DOWN, piece, collision)
...
class Tetris:
    ...
    def game_logic(self):
            ...
            for x, y in piece_positions:
                if (x, y) not in self.landed:
                    try:
                        ...
                        print('crushed through the wall')
        ...
        """ # do some stuff if piece lands
        if self.change_piece:
        
        this moves to AI.py, if a piece needs to be changed, prepare a new state 
        
            
        # update ai field
        ai.update_ai(current_piece)

        ai.get_best_move()

        best_move = ai.best_move

        make_ai_move(current_piece, best_move, collision)

        hueris.update_field(ai.field)
        current_fitness += hueris.get_fitness()
        """
        self.draw_window()
    ...
    def make_ai_move(self):  # collision is an obj
        current_config = self.current_piece.get_config()  # exp : (3, (7, 21), '[(7, 20), (8, 20), (9, 20), (7, 21)]', False, inf, inf, None)
        target_config = self.best_move  # exp: (0, (7, 21), '[(6, 19), (7, 19), (7, 20), (7, 21)]', False, inf, inf, None)
        ...
        if cu_rot_state == t_rot_state:
            moves = t_x - cu_x
            if t_x > cu_x:
                for _ in range(abs(moves) - 1):
